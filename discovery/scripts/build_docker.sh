...
# 현재 모듈(discovery)의 jar를 이용해 Docker 이미지만 만든다.
...
echo "[discovery] 도커 이미지 빌드 시작: ${IMAGE}"
docker build -t "${IMAGE}" .

if [[ "${LOAD_TO_MINIKUBE}" == "true" ]]; then
  echo "[discovery] minikube에 이미지 적재: ${IMAGE}"
  minikube image load "${IMAGE}"
fi

if [[ "${PUSH_IMAGE}" == "true" ]]; then
  echo "[discovery] 원격 레지스트리로 이미지 푸시: ${IMAGE}"
  docker push "${IMAGE}"
fi

echo "[discovery] 도커 이미지 생성 완료: ${IMAGE}"
